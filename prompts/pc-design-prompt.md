...
- 默认产出静态 HTML 设计稿，同时引入 Tailwind Play CDN 和 `dist/pc-design.css` 编译 CSS。
...
- 若生成静态 HTML，优先按文件位置正确引入 Tailwind Play CDN 和 `dist/pc-design.css`，并使用已有语义类；确需自定义样式时，优先使用 Tailwind utility class，再考虑 `tokens/pc-design.tokens.css` 中的变量。
